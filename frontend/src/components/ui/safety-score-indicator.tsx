...
export function SafetyScoreIndicator({ score, label, tone = "neutral", detail, compact = false, className }: {
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      <div className={cn("grid shrink-0 place-items-center rounded-full border-4 border-[var(--color-primary-soft)] bg-white font-[family-name:var(--font-display)] font-bold text-[var(--color-ink)]", compact ? "size-12 text-sm" : "size-16 text-lg")}>{score}</div>
